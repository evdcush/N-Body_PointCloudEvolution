...
def get_csr_periodic_bc(X_in, K, shell_fraction=0.1):
    ...
    for i in range(mb_size):
        ...
        #graph_idx = kneighbors_graph(new_X[:, :3], K, include_self=True)[:N, :].indices.reshape([-1, K]) # CONFIRMED
        #csr = kneighbors_graph(new_X[:, :3], K, include_self=True)
        graph_idx = kneighbors_graph(new_X[:, :3], K, include_self=True).indices.reshape([-1, K])[:N,:]
        #trunc0 = _truncateOG(csr, N, K, ids_map)
        #trunc1 = _truncateLIL(csr, N, K, ids_map)
        #code.interact(local=dict(globals(), **locals())) # DEBUGGING-use
        for j in range(N):
            for k in range(K):
                if graph_idx[j, k] > N - 1:  # If outside of the box
...
def _get_csr_periodic_bc(X_in, K, shell_fraction=0.1):
    """
    Map inner chunks to outer chunks
    cant black box this anymore
    NEED TO CLEAN THIS UP, at least var names
    ...
    for i in range(mb_size):
        ...
        new_X = np.array(new_X)
        '''
        graph_csr = kneighbors_graph(new_X[:, :3], K, include_self=True)
        graph_idx = graph_csr.indices.reshape([-1, K])
        graph_idx_box = graph_idx[:N, :]
        IS THE SAME AS:
        graph_idx_box = kneighbors_graph(new_X[:, :3], K, include_self=True)[:N, :].indices.reshape([-1, K])

        '''
        graph_csr = kneighbors_graph(new_X[:, :3], K, include_self=True)[:N, :]
        graph_idx = graph_csr.indices.reshape([-1, K])
        graph_lil = graph_csr.tolil()
        graph_lil_box = graph_lil[:N,:]
        #for j in range(N): graph.rows[j] = [r if r < N else ids_map[r] for r in graph.rows[j]]
        print('originall')
        code.interact(local=dict(globals(), **locals())) # DEBUGGING-use
...
def get_csr_periodic_bc2(X_in, K, shell_fraction=0.1):
    ...
    for i in range(mb_size):
        ...
        graph = kneighbors_graph(new_X[:,:3], K, include_self=True)[:N,:].tolil()
        #code.interact(local=dict(globals(), **locals())) # DEBUGGING-use
        for j in range(N):
            cur_row = graph.rows[j]
            graph.rows[j] = [r if r < N else ids_map[r] for r in cur_row]
        graph_idx = np.reshape(graph.tocsr().indices, (N, K))
        #graph_idx = np.reshape(graph[:,:N].tocsr().indices, (N, K))
        adj_list[i] = graph_idx
        #csr_list.append(graph_csr)#, np.diff(graph_csr.indptr)]
    return adj_list

alist_og = get_csr_periodic_bc(np.copy(x), K, shell_fraction=0.1)
