...
def ShiftInv_layer(H_in, adj, bN, layer_id, is_last=False):
    ...
    symmetrized_i = number of non-zero entries of symmetrized adjacency. If adjacency is symmetric, then
    symmetrized_i = N*M, but in general it's not. Also, even if all instances in the batch have a fixed number of
    neighbors (i.e. all unsymmetrized adjacencies have N*M non-zero entries), the corresponding symmetrized
    versions can contain different number of entries. Our implementation where all dimensions but channels are flattened
...
# Updated/Corrected Shiftinv layer
# ===================================
def ShiftInv_layer_AdjTensor(H_in, adj, adj_diags, bN, layer_id, is_last=False):
    """
    Same as other updated shiftinv, but with tensor feed rather than dict
...
def prev_ShiftInv_layer(H_in, COO_feats, bN, layer_id, is_last=False):
...
def get_readout_within_range(h_out, upper=32.0, lower=0.0):
    M = h_out.get_shape().as_list()[-1]
